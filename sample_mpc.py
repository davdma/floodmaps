...
def event_completed(dir_path):
    ...
    regex_patterns = ['dem_.*\.tif', 'flowlines_.*\.tif', 'roads_.*\.tif', 'slope_.*\.tif', 'waterbody_.*\.tif', 'tci_\d{8}.*\.tif', 'ndwi_\d{8}.*\.tif', 'b08_\d{8}.*\.tif', 'metadata.json']
    pattern_dict = {'dem_.*\.tif': 'DEM', 'flowlines_.*\.tif': 'FLOWLINES', 'roads_.*\.tif': 'ROADS', 'slope_.*\.tif': 'SLOPE', 'waterbody_.*\.tif': 'WATERBODY', 'tci_\d{8}.*\.tif': 'S2 IMAGERY', 'ndwi_\d{8}.*\.tif': 'NDWI', 'b08_\d{8}.*\.tif': 'B8 NIR', 'metadata.json': 'METADATA'}
...
def main(threshold, days_before, days_after, maxcoverpercentage, maxevents, dir_path=None):
    """
    ...
    try:
        for event_date, event_precip, minx, miny, maxx, maxy, eid in events:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    if event_sample(threshold, days_before, days_after, maxcoverpercentage, event_date, event_precip, minx, miny, maxx, maxy, eid, dir_path):
                        count += 1
                    history.add(f'{threshold}_{eid}')
                    break
...
if __name__ == '__main__':
    ...
    sys.exit(main(args.threshold, args.days_before, args.days_after, args.maxcoverpercentage, args.maxevents, dir_path=args.dir_path))
