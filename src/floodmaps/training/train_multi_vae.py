...
from floodmaps.utils.utils import (flatten_dict, ADEarlyStopper, Metrics, BetaScheduler, get_gradient_norm,
                   get_model_params, print_model_params_and_grads, load_model_weights)
...
def train_loop(model, dataloader, device, optimizer, loss_fn, cfg, run, epoch, beta_scheduler=None):
    """Training loop for VAE with posterior collapse monitoring.
...
def test_loop(model, dataloader, device, loss_fn, cfg, run, epoch, beta_scheduler=None):
    ...
    with torch.no_grad():
        for batch_i, (X, y) in enumerate(dataloader):
            ...
            # Ablation diagnostic on first batch only (VAE decode has no conditioning)
            if batch_i == 0:
                out_mu = model.decode(mu)
                loss_mu = loss_fn(out_mu, y)
                z_zero = torch.zeros_like(mu)
                out_zero = model.decode(z_zero)
...
def run_experiment_ad(cfg):
    ...
    if hasattr(cfg.model, 'weights') and cfg.model.weights is not None:
        load_model_weights(model, cfg.model.weights, device, model_name="Autodespeckler")
