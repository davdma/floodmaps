...
def sample_patches_in_mem(tiles: List[Tuple], size: int, num_samples: int, cloud_threshold: float,
                          filter_type: str, seed: int, max_attempts: int = 4000) -> np.ndarray:
    ...
    for i, tile in enumerate(tiles):
        ...
        if patches_sampled < num_samples:
            _, _, _, eid, img_dt = tile
            raise RuntimeError(f"Worker exceeded max retries {max_attempts}, only sampled {patches_sampled}/{num_samples} patches for tile {i} (eid: {eid}, dt: {img_dt})")

    return dataset


def sample_patches_in_disk(tiles: List[Tuple], size: int, num_samples: int, cloud_threshold: float,
                          filter_type: str, seed: int, scratch_dir: Path, worker_id: int, max_attempts: int = 1000) -> Path:
    ...
    try:
        for i, tile in enumerate(tiles):
            ...
            if patches_sampled < num_samples:
                _, _, _, eid, img_dt = tile
                raise RuntimeError(f"Worker {worker_id} exceeded max retries {max_attempts}, only sampled {patches_sampled}/{num_samples} patches for tile {i} (eid: {eid}, dt: {img_dt})")
    finally:
