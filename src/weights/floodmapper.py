...
def get_standardize_s2(cfg):
    """Get standardization transform for S2 data. Need to refactor for
    new S2 preprocessing."""
    channels = [bool(int(x)) for x in cfg.data.channels]
    b_channels = sum(channels[-2:])
    with open(SRC_DIR / f'data/s2/stats/{cfg.data.size}_{cfg.data.samples}.pkl', 'rb') as f:
...
def get_standardize_s1(cfg):
    ...
    channels = [bool(int(x)) for x in cfg.data.channels]
    b_channels = sum(channels[-2:])
    with open(SRC_DIR / f'data/sar/stats/minibatch_{cfg.data.filter}_{cfg.data.size}_{cfg.data.samples}.pkl', 'rb') as f:
        train_mean, train_std = pickle.load(f)
